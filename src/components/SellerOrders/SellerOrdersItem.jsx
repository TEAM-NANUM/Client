...
const SellerOrdersItem = ({item, datas, PROXY}) => {
    ...
    const onProgress = () => {
        if(item.delivery_status === "PAYMENT_COMPLETE") {
            axios.patch(`${PROXY}/api/seller/order/${item.id}`, {
            ...
            <div className='SellerOrdersItem_info'>
                ...
                <div className='SellerOrdersItem_content'>
                    ...
                    <div className='SellerOrdersItem_content_info'>
                        <span>주문 정보 : </span>
                        <div className='SellerOrdersItem_item_info'>{datas.products.name} {datas.products.unit}kg {item.quantity}개</div>
                        <span>합계 : </span>
                        <div className='SellerOrdersItem_item_totalprice'>{(datas.products.price * item.quantity).toLocaleString()} 원</div>
                    </div>
                </div>
            </div>
            <div className='SellerOrdersItem_deliveryState'>
                <div className={item.delivery_status === "PAYMENT_COMPLETE" || item.delivery_status === "IN_PROGRESS" || item.delivery_status === "DELIVERED" ? 'deliveryState_state_active' : 'deliveryState_state'}>결제 완료</div>
                <div className='deliveryState_arrow'>&nbsp; → &nbsp;</div>
                <div className={item.delivery_status === "IN_PROGRESS" || item.delivery_status === "DELIVERED" ? 'deliveryState_state_active' : 'deliveryState_state'} onClick={onProgress}>배송중</div>
                <div className='deliveryState_arrow'>&nbsp; → &nbsp;</div>
                <div className={item.delivery_status === "DELIVERED" ? 'deliveryState_state_active' : 'deliveryState_state'} onClick={onDelivered}>배송 완료</div>
            </div>
        </div>
    );
};
