...
const MainImg = ({
  ...
  const onSetMainImg = (e) => {
    ...
    axios
      ...
      .then((res) => {
        axios
          .put(res.data.URLlist[0].presigned_url, e.target.files[0], {
            headers: {
        ...
        setProductForm({
          ...productForm,
          img_url: res.data.URLlist[0].image_url,
        });
      })
      .then((res) => {
        console.log(res);
  ...
  return (
    <div className="MainImg_container">
      ...
      {!mainImg && (
        <input type="file" accept="image/*" onChange={onSetMainImg} />
      )}
      {mainImg && <img src={mainImg} alt="상품 메인 사진" />}
      <div className="img_change_btn" onClick={onImgChange}>
        대표사진 수정하기
      </div>
    </div>
  );
