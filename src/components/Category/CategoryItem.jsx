...
const CategoryItem = ({category_name, category_id, PROXY}) => {
    ...
    const onProductListCategory = (e) => {
      navigate(`/search?category=${e.currentTarget.id}&name=${encodeURIComponent(e.currentTarget.className)}`)
    }
    ...
    return (
        <div className='categoryItem_container'>
          <div className='categoryItem'><span className={category_name} id={category_id} onClick={onProductListCategory}>- 전체</span></div>
            {categoryItem.subcategories.map((item, idx) => 
                  <div className='categoryItem'>
                    <span className={item.name} id={item.id} onClick={onProductListSub}>- {item.name}</span>
                  </div>
            )}
        </div>
    );
