...
const SellerOrdersPage = ({PROXY, productID}) => {
    ...
    useEffect(() => {
        ...
        if(!productID) {
            ...
            axios.get(`${PROXY}/api/seller/${productID}`, {
                headers: {
    ...
    return (
        <>
            ...
            <div className='SellerOrdersPage_container'>
                {productOrderData && productOrderData.orders.map((item, idx) => <SellerOrdersItem key={idx} item={item} datas={productOrderData} PROXY={PROXY} />)}
            </div>
        </>
    );
};
