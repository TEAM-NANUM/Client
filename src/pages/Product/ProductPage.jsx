  ...
  return (
    <>
      ...
      <div className="ProductPage_top">
        <div>
          <div
            className="prod_category_select_btn"
            onClick={() => {
              navigate("/category");
            }}
          >
            {searchParams.get("name") || "카테고리"}
          </div>
        </div>
        <div className="product_sort_btn" onClick={handleSortToggle}>
          {searchParams.get("sort") === "recent"
            ...
            ? "인기순"
            : searchParams.get("sort") === "review"
            ? "후기순"
            : searchParams.get("sort") === "rating"
            ? "평점순"
            : "최신순"}
          <img
            src={`./img/imgProduct/sortDrop${showSortPopup ? "On" : "Off"}.svg`}
            alt="Back"
          />
        </div>
        {showSortPopup && (
      ...
      {product && (
        <div className="Product_list">
          {!isLoading && infoSearch ? (
            <div style={{ display: "flex", width: "100%" }}>
              <img
                style={{ margin: "40px auto 0 auto" }}
                src={`./img/imgProduct/pleaseSearch.svg`}
                alt="검색해주세요"
              />
            </div>
          ) : (
          ...
          {!isLoading && product.length === 0 && !infoSearch ? (
            <div style={{ display: "flex", width: "100%" }}>
              <p style={{ margin: "40px auto 0 auto", color: "#707070" }}>
