...
const AddressPage = ({ PROXY, addressList, setAddressList, setFixNum }) => {
  ...
  const getAddress = () => {
    axios
      .get(`${PROXY}/api/delivery-address`, {
        headers: {
  ...
  return (
    <>
      <SubHeader
        page={"배송지 관리"}
        path={searchParams.get("purchase") === "true" ? "/purchase" : "/mypage"}
      />
      {addressList.delivery_address.length !== 0 ? (
        <AddressList
        ...
        <img
          src="./img/imgAddress/noAddress.svg"
      ...
      >
        <div
          ...
          onClick={() =>
            navigate(
              "/addressAdd?purchase=" + searchParams.get("purchase") || ""
            )
          }
        >
          <p>주소 추가</p>
