...
const PurchasePage = ({
  ...
  const doPay = () => {
    ...
    if (purchaseDetail.length === 1) {
      ...
      axios
        .post(`${PROXY}/api/orders`, requestData, {
          headers: {
      ...
      axios
        .post(`${PROXY}/api/cart/purchase`, requestData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
          },
        })
  ...
  useEffect(() => {
    ...
    if (userData === undefined || purchaseDetail === null) {
      ...
      if (
        userInfo &&
        userInfo.point -
          purchaseDetail.reduce((total, item) => {
            return total + item.price * item.quantity;
          }, 0) <
          0
      ) {
      ...
      {userInfo && innerPurchaseDetail && (
        <>
          <SubHeader
            page={"주문 작성"}
            path={
              purchaseDetail.length === 1
                ? `/productDetail/${purchaseDetail[0].productId}`
                : "/shoppingCart"
            }
          ></SubHeader>
          <div
            className="divider"
            style={{ marginTop: "53px", marginBottom: "10px" }}
          />
          <div
            style={{
              display: "flex",
          ...
          >
            <span
              style={{ fontSize: "15px", fontWeight: "600", padding: "0 16px" }}
              className="pay_span"
            >
              배송지
            </span>
          ...
          <div className="address_man_item_wrapper" style={{ margin: "6px 0" }}>
            <div
              className="address_man_item_name"
              style={{ fontSize: "12px", fontWeight: "500" }}
            >
              <div>{`${userInfo.user_name}`}</div>
            </div>
            <div
              className="address_man_item_phone"
              style={{ fontSize: "10px", color: "#5e5e5e" }}
            >
              {userInfo.phone_number}
            </div>
            <div
              className="address_man_item_address"
              style={{ fontSize: "11px" }}
            >{`${userInfo.address.default_address} ${userInfo.address.detail_address}`}</div>
          </div>
          <div
            className="divider"
          ...
          {innerPurchaseDetail.map((product, index) => (
            <>
              <div
                className="ShoppingCartList_content"
                style={{ height: "110px", cursor: "default" }}
              >
                <img
                  className="cart_product_img"
                  src={product.imgUrl}
                  alt="상품이미지"
                  style={{ margin: "0 8px 0 16px", cursor: "default" }}
                ></img>
                <div
              ...
              <div
                ...
                style={{
                  marginTop: "10px",
                  marginBottom: "10px",
                  height: innerPurchaseDetail.length - 1 === index ? "" : "2px",
                }}
              />
            </>
          ))}
            ...
            >
              ...
              <p
                style={{
                  ...
                  margin: "7px 0",
                  fontSize: "14px",
                  color: pointErr ? "#ff3535" : "#292929",
                }}
              >
                {makeTotalPrice(true)} 원
              </p>
            </div>
            <div
              style={{
          ...
          <div
            ...
            style={{ flexDirection: "column", height: payErr ? "92px" : "" }}
          >
            {payErr && (
              <p
                style={{
